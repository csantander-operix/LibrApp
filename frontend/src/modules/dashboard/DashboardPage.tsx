...
const kpis = [
  { key: "total_libros", label: "Libros en catálogo", icon: BookOpen, color: "text-unla" },
  { key: "total_estantes", label: "Estantes", icon: Map, color: "text-blue-600" },
  { key: "total_colecciones", label: "Colecciones", icon: Library, color: "text-emerald-600" },
  { key: "libros_sin_ubicar", label: "Sin ubicar", icon: MapPinOff, color: "text-amber-600" },
] as const;
  ...
  return (
    <div>
      <header className="mb-6">
        ...
        <p className="text-sm text-slate-500">Resumen del catálogo de la Librería Rodolfo Walsh.</p>
      </header>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {kpis.map(({ key, label, icon: Icon, color }) => (
            <Card key={key} className="flex items-center gap-4">
              <div className={`flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 ${color}`}>
      ...
      <Card className="mt-6">
        <h2 className="text-sm font-semibold text-slate-700">Funcionalidades disponibles</h2>
